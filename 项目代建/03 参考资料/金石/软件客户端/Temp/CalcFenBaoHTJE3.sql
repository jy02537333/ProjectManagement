...
BEGIN
  ...
  Set @Sql='Declare cur1 Cursor Global For Select Sum(金额) As LjJE From 收付款总账表 where 冲销=0 And 单据类型=''收款单'' And 执行合同=''承包合同'' And 收付款名称=''工程款'' And 合同编号 In (Select 合同编号 From 承包合同表 where XMPathkey='''+@XMPathKey+''' )';
  Set @Sql=@Sql+@FKDateSql 
  Exec(@Sql)
  Open cur1
